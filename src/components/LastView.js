...
const LastView = () => {
  ...
  return (
    <div>
      <Typography sx={{ display: 'grid', placeItems: 'center', marginTop: '2rem' }} component='span' variant='h4' color='text.primary'>
        ...
        lastUserViews?.lastViews.map(item => {
          ...
          return (
            <List sx={{ width: '100%', maxWidth: 360, bgcolor: 'background.paper' }} key={item.id}>
              <ListItem alignItems='flex-start' onClick={() => navigate(`detail/${item.id}`)}>
                <ListItemAvatar>
